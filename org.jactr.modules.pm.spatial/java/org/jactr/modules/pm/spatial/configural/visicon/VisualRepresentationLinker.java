...
public class VisualRepresentationLinker extends ActivationBufferListenerAdaptor
{
  ...
  final private IConfiguralModule    _configuralModule;
  
  public VisualRepresentationLinker(IVisualModule visualModule,
      IConfiguralModule configuralModule)
  {
    _visualModule = visualModule;
    _configuralModule = configuralModule;
  }
  ...
  public void attach()
  {
    ...
    _configuralModule.getConfiguralBuffer().addListener(this,
        ExecutorServices.INLINE_EXECUTOR);
  }
  ...
  public void detach()
  {
    ...
    _configuralModule.getConfiguralBuffer().removeListener(this);
  }
  ...
  public void sourceChunkAdded(ActivationBufferEvent bufferEvent)
  {
    ...
    {
      visualChunk = added;
      configuralChunk = findMatch(_configuralModule.getConfiguralBuffer(),
          identifier);
    }
    else
    {
      configuralChunk = added;
      visualChunk = findMatch(_visualModule.getVisualActivationBuffer(),
          identifier);
    }
    
    if (visualChunk == null || configuralChunk == null)
    {
      if (LOGGER.isDebugEnabled())
        LOGGER.debug("Could not find both visual(" + visualChunk
            + ") and configural(" + configuralChunk
    ...
    {
      String msg = "Connecting " + configuralChunk + " and " + visualChunk;
      if (Logger.hasLoggers(model))
        Logger.log(model, IConfiguralModule.CONFIGURAL_LOG, msg);
