...
public abstract class AbstractImaginedTransformSource implements
    ITransformSource
{
  /**
   * Logger definition
  ...
  static private transient Log    LOGGER = LogFactory
                                             .getLog(AbstractImaginedTransformSource.class);

  AbstractTimedEvent              _expirationEvent;
  ...
  public AbstractImaginedTransformSource(final IConfiguralModule module,
      double[] amount, double[] rate, double startTime)
  {
    ...
    _expirationEvent = new AbstractTimedEvent(startTime, endTime) {
      @Override
      public void fire(double time)
      {
        super.fire(time);
        if (LOGGER.isDebugEnabled()) LOGGER.debug("Stopping transformation @ "+time);
        module.getImaginedPathIntegrator().stop();
      }
      ...
      {
        super.abort();
        // called in response to IConfiguralModule.reset()
        // or the zeroing of the buffer
        module.getImaginedPathIntegrator().abort();
      }
    };
  ...
  protected double[] getTransformationSince(double lastTime, double now)
  {
    ...
    /*
     * in case the amount of time to perform this transformation is actually
     * less than the production firing time..
     */
    now = Math.min(now, _expirationEvent.getEndTime());
