...
public abstract class AbstractManipulativeActivationBuffer extends
    AbstractCapacityPMActivationBuffer6 implements IManipulativeBuffer
{
  /**
   * logger definition
   */
  static public final Log LOGGER = LogFactory
                                     .getLog(AbstractManipulativeActivationBuffer.class);

  private Set<IChunk>     _matchedChunks;

  public AbstractManipulativeActivationBuffer(String name,
  ...
  protected void chunkInserted(IChunk insertedChunk)
  {
    if (insertedChunk.isEncoded())
      throw new RuntimeException("Inserted a previously encoded chunk? WTF? "
          + insertedChunk);

    IChunk restoreState = null;

    if (!isIntegratorFree())
      restoreState = (IChunk) getSlot(IStatusBuffer.STATE_SLOT)
          .getValue();

    /*
     * this will reset the state to free, but if integrator is busy or error, we
     * will need to reset the state to whatever it was (busy most likely)
     */
...
//      if (((IManipulativeModule) getModule()).isEncodeUpdatedChunksEnabled()
